...
const steps = [
  { number: "01", title: "30 Minutes Consultancy", desc: "We discuss your business goals, challenges, and what you actually need to achieve your objectives.", icon: MessageSquare, illustration: stepConsultation },
  { number: "02", title: "Requirement & Analysis", desc: "We assess your project requirements and gather necessary assets. We analyze everything based on your specific needs.", icon: Search, illustration: stepAnalysis },
  { number: "03", title: "Implementation", desc: "We build your project with precision, implementing all features and functionalities according to the agreed requirements.", icon: CodeXml, illustration: stepImplementation },
  { number: "04", title: "Revision", desc: "We review the project together, make necessary adjustments, and ensure everything meets your expectations.", icon: RefreshCw, illustration: stepRevision },
  { number: "05", title: "Project Delivery", desc: "Your project is delivered with complete documentation and support for a smooth launch.", icon: Rocket, illustration: stepDelivery },
];
    ...
    <div ref={cardRef} className="relative mb-16 last:mb-0">
      {index < steps.length - 1 && (
        <div className="hidden md:block absolute left-1/2 top-12 bottom-0 w-px -translate-x-1/2">
          ...
          <motion.div className="absolute top-0 left-0 w-full bg-accent" style={{ height: lineHeight }} />
        </div>
      )}
      ...
      >
        <div className="rounded-2xl border border-border bg-card p-6 md:p-8 hover:border-accent/40 transition-colors duration-300">
          ...
          <div className="mt-4 flex items-center gap-2">
            {steps.map((_, i) => (
              <div key={i} className={`h-1 flex-1 rounded-full ${i <= index ? "bg-accent" : "bg-border"}`} />
            ))}
          </div>
        </div>
      </motion.div>
